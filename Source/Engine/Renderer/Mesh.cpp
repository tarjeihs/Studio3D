...
void CMesh::UploadAssetData(const std::vector<glm::vec3>& Vertices, const std::vector<glm::vec3>& Normals, const std::vector<glm::vec2>& UV0, const std::vector<uint32>& Indices)
{
    ...
    MeshAsset.VAO->LinkAttrib(*MeshAsset.VBO, 0, 3, GL_FLOAT, 8 * sizeof(float), (void*)0);                       // Position Attribute
    MeshAsset.VAO->LinkAttrib(*MeshAsset.VBO, 1, 3, GL_FLOAT, 8 * sizeof(float), (void*)(3 * sizeof(float)));     // Normal Attribute
    MeshAsset.VAO->LinkAttrib(*MeshAsset.VBO, 2, 2, GL_FLOAT, 8 * sizeof(float), (void*)(6 * sizeof(float)));     // UV0 Attribute
